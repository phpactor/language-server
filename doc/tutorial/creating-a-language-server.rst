...
The dispatcher is responsible for bootstrapping your and creating all the
necessary classes that you will need. As a foundation it is passed two
necessary dependencies from the language server itself; the
``MessageTransmitter`` and the ``InitializeParams``.
...
- ``RpcClient`` which allows you to send *requests* and *notifications* to
  the client.
- ``ServiceProviders``, ``PingProvider``, ``ServiceManager``: Ping provider is an annoying
  service which pings your client for no reason at all, it is a background
  process. See  :doc:`../reference/service-providers` for more information on
  services.
- ``EventDispatcher``: Required by some middlewares to transmit events which
  ...
  We use a ``ServiceListener`` here which is responsible to start all the
  services when the server is ``initialized``.
